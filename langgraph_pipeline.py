...
def _json_to_df(json_str: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "Error": [
                f"Failed to deserialize result. Original content: {json_str[:50]}..."
            ]
        }
    )
...
def setup_llm_client(config: Config) -> AzureOpenAI:
    ...
    try:
        ...
        print(
            f"[INFO] AzureOpenAI Client configured for deployment: {config.azure_openai.llm_deployment_name}"
        )
        return client
...
def decompose_query(state: GraphState, llm_client: AzureOpenAI) -> GraphState:
    ...
    try:
        ...
        state["sub_queries"] = [state["user_question"]]
        return state
...
def identify_data_sources(state: GraphState, llm_client: AzureOpenAI) -> GraphState:
    print("\n" + "-" * 80)
    print(
        f"NODE: Identify Data Sources (Query {state['query_index'] + 1}/{len(state['sub_queries'])})"
    )
    print("-" * 80)
        ...
        if "*" in required_tables or required_tables == ["*"]:
            ...
            for table_name in required_tables:
                if table_name in state["global_catalog_dict"]:
                    target_files.append(
                        state["global_catalog_dict"][table_name]["parquet_path"]
                    )

            state["target_parquet_files"] = (
                target_files if target_files else state["all_parquet_files"]
            )
            state["use_union_by_name"] = len(target_files) > 1 and not join_key
            mode = "UNION" if state["use_union_by_name"] else "JOIN"

        print(
            f"[SUCCESS] Tables: {required_tables} -> Files: {len(state['target_parquet_files'])} ({mode})"
        )
...
"""
        response = llm_client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"User Query: {state['current_query']}. Output the classification in JSON format.",
                },
            ],
            temperature=0.0,
        ...
        state["intent"] = (
            intent if intent in ["SEMANTIC_SEARCH", "SQL_QUERY"] else "SQL_QUERY"
        )
...
def perform_semantic_search(state: GraphState, llm_client: AzureOpenAI) -> GraphState:
    ...
    try:
        ...
        if retrieved_files:
            ...
            print(
                f"[INFO] Updated target files based on semantic search: {len(retrieved_files)} files"
            )
        ...
        if state["required_tables"] and state["required_tables"] != ["*"]:
            for logical_name in state["required_tables"]:
                if logical_name in state["global_catalog_dict"]:
                    path_map[logical_name] = state["global_catalog_dict"][logical_name][
                        "parquet_path"
                    ]
...
def execute_query(state: GraphState, llm_client: AzureOpenAI) -> GraphState:
    ...
    try:
        ...
        result_df = execute_duckdb_query(
            query=state["sql_query"], config=state["config"]
        )
...
def should_continue_processing(
    state: GraphState,
) -> Literal["identify_sources", "generate_summary"]:
    """
...
def create_rag_graph(llm_client: AzureOpenAI) -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: (
            "semantic_search"
            if state["intent"] == "SEMANTIC_SEARCH"
            else "generate_sql"
        ),
        {"semantic_search": "semantic_search", "generate_sql": "generate_sql"},
    )
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_file = {
            ...
            for excel_path in config.input_file_paths
    ...
    global_catalog_string, global_catalog_dict = build_global_catalog(
        all_parquet_files, config
    )
